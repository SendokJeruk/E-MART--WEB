    ...
    <div class="flex justify-center items-center min-h-screen bg-[#eeeeee] p-4">
        <div
            ...
            <div v-if="isLoading" class="flex flex-col items-center w-full py-4">
                ...
                <p class="text-lg">Mengirim tautan...</p>
...
const sendResetLink = async () => {
    ...
    try {
        ...
        if (response.data.status === "Success" || response.status === 200) {
            showSuccess(response.data.message || "Tautan reset password telah dikirim ke email Anda.")
            email.value = ''
        } else {
            showError(response.data.message || "Gagal mengirim tautan reset.")
        }
    } catch (error) {
        console.error(error)
        showError("Gagal mengirim tautan. Pastikan email terdaftar atau coba lagi nanti.")
    } finally {
