<template>
  <adminside>
    <!-- Halaman Manajemen User: Tempat admin melihat, mencari, dan mengelola pengguna aplikasi -->
      ...
      <div class="overflow-x-auto rounded-lg shadow border border-gray-200">
        <table class="min-w-[500px] w-full table-auto divide-y divide-gray-200 text-xs sm:text-sm">
          <thead class="bg-gray-100">
            <tr>
              <th class="px-4 py-3 text-left navbar-font text-gray-700 whitespace-nowrap">Name</th>
              <th class="px-4 py-3 text-left navbar-font text-gray-700 whitespace-nowrap">Email</th>
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            <template v-if="isLoading">
              <tr v-for="n in 6" :key="n">
                <td class="px-4 py-3"><Skeleton height="14px" width="80%" /></td>
                <td class="px-4 py-3"><Skeleton height="14px" width="90%" /></td>
                <td class="px-4 py-3"><div class="flex flex-wrap gap-2"><Skeleton height="30px" width="60px" /><Skeleton height="30px" width="60px" /></div></td>
              </tr>
            </template>

            <!-- Tampilan List User Asli -->
            <template v-else-if="users.length > 0">
              <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 transition">
                <td class="px-4 py-3 font-medium inter-font text-gray-900 whitespace-nowrap">{{ user.name }}</td>
                <td class="px-4 py-3 inter-font text-gray-700 break-all">{{ user.email }}</td>
                <td class="px-4 py-3">
                  <div class="flex flex-wrap gap-2">
                    <button @click="deleteUser(user.id)" class="px-3 py-1 text-xs sm:text-sm bg-red-500 hover:bg-red-600 text-white rounded navbar-font transition">Hapus</button>
                    <router-link :to="`/edit-user/${user.id}`" class="px-3 py-1 text-xs sm:text-sm bg-yellow-500 hover:bg-yellow-600 text-white rounded navbar-font transition">Edit</router-link>
                  </div>
                </td>
              </tr>
            </template>

            <!-- Tampilan jika tidak ada user ditemukan -->
            <template v-else>
              <tr><td colspan="3" class="px-4 py-8 text-center text-gray-500">Tidak ada data user</td></tr>
            </template>
          </tbody>
        </table>
      </div>
...
/**
 * Mengambil data pengguna dari server berdasarkan halaman dan kata kunci pencarian.
 */
const fetchUsers = async (page = 1) => {
  try {
    isLoading.value = true
    const response = await api.get('/manage-user', { params: { page: page, name: searchQuery.value } })
