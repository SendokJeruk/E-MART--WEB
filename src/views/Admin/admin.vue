  ...
  <adminside>
    <div class="p-6 bg-[#F9FAFB] min-h-screen">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <template v-else>
          <div class="bg-white rounded-xl shadow-sm border p-6 text-center">
            <h2 class="font-medium text-gray-600 mb-2">Total GMV</h2>
            <p class="text-2xl font-bold text-red-700">Rp {{ summary.total_gmv.toLocaleString('id-ID') }}</p>
          </div>
          <div class="bg-white rounded-xl shadow-sm border p-6 text-center">
            <h2 class="font-medium text-gray-600 mb-2">Pengguna Baru</h2>
            <p class="text-2xl font-bold text-red-700">{{ summary.total_new_users }}</p>
          </div>
          <div class="bg-white rounded-xl shadow-sm border p-6 text-center">
            <h2 class="font-medium text-gray-600 mb-2">Transaksi Sukses</h2>
            <p class="text-2xl font-bold text-red-700">{{ summary.total_success_transactions }}</p>
          </div>
          <div class="bg-white rounded-xl shadow-sm border p-6 text-center">
            <h2 class="font-medium text-gray-600 mb-2">Toko Terdaftar</h2>
            <p class="text-2xl font-bold text-red-700">{{ summary.total_stores }}</p>
          </div>
        </template>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border p-6 mb-8">
        <h2 class="text-lg font-semibold text-gray-700 mb-4">Tren GMV ({{ groupingType }})</h2>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
          <h2 class="text-lg font-semibold text-gray-700 mb-4">Pertumbuhan Pengguna Baru</h2>
        ...
        <div class="bg-white rounded-xl shadow-sm border p-6 overflow-hidden flex flex-col">
          <h2 class="text-lg font-semibold text-gray-700 mb-4">Top 5 Seller</h2>
            ...
            <table class="min-w-full text-sm text-left">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="(seller, index) in topSellers" :key="index" class="hover:bg-gray-50 transition">
                  <td class="px-4 py-3 font-semibold text-gray-800">{{ seller.nama_toko }}</td>
                  <td class="px-4 py-3 text-right text-red-600 font-medium">Rp {{ Number(seller.total_pendapatan).toLocaleString('id-ID') }}</td>
                  <td class="px-4 py-3 text-center text-gray-600">{{ seller.total_pesanan }}</td>
                </tr>
