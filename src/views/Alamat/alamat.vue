<template>
  ...
  <!-- Halaman Alamat: Tempat user mengelola daftar alamat pengiriman mereka -->
  <div class="mt-10 px-6 space-y-6">
    <h2 class="text-2xl navbar-font text-[#7D0A0A]">Daftar Alamat Anda</h2>

    <!-- Pesan jika belum ada alamat -->
    <div v-if="alamatUser.length === 0" class="text-gray-600 inter-font">
      Belum ada alamat yang ditambahkan.
    </div>

    <!-- Perulangan untuk menampilkan setiap alamat -->
    <div
    ...
    >
      <div>
        ...
        <!-- Kode Domestik untuk integrasi pengiriman/kurir -->
      ...
      <div class="flex justify-end gap-3 mt-4">
        <router-link :to="`/edit-alamat/${alamat.id}`" class="px-4 py-1.5 text-sm bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition">
          Edit
        </router-link>
        <button @click="deleteAlamat(alamat.id)" class="px-4 py-1.5 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
          Hapus
        </button>
      </div>
    </div>
    ...
    <div class="pt-4">
      <router-link to="/create-alamat" class="group relative inline-block focus:outline-none">
        <span class="absolute inset-0 border-2 border-[#BF3131] rounded-full"></span>
        <span class="block rounded-full bg-[#BF3131] px-8 py-3 text-sm font-bold text-white transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1">
          Tambah Alamat
        </span>
      </router-link>
    </div>
...
/**
 * Mengambil daftar alamat milik pengguna dari API.
 */
...
/**
 * Menghapus alamat tertentu setelah mendapatkan konfirmasi dari user.
 */
const deleteAlamat = async (id) => {
  ...
  try {
    ...
    // Hapus data dari tampilan tanpa harus reload halaman
